...
import { Hammer, Sword, Shield, Wrench, Coins, Flame } from 'lucide-react';
...
const SmithyLocation: React.FC = () => {
  ...
  const craftableWeapons = [
    ...
    },
    {
      ...
      cost: 250,
      materials: ['Iron Ore: 4', 'Wood: 2'],
      stats: { strength: 10, dexterity: -2 },
    },
  ];

  const craftableArmor = [
    {
      id: 'chainmail',
      name: 'Chainmail Armor',
      type: 'armor',
      description: 'Flexible metal armor offering good protection',
      cost: 180,
      materials: ['Iron Ore: 5', 'Leather: 2'],
      stats: { vitality: 6 },
    },
    {
      id: 'plate_armor',
      name: 'Plate Armor',
      type: 'armor',
      description: 'Heavy steel armor providing maximum protection',
      cost: 300,
      materials: ['Steel Ingot: 3', 'Leather: 3'],
  ...
  const handleRepairUpgrade = async (item: InventoryItem, serviceType: 'repair' | 'upgrade') => {
    ...
    try {
      // Use real API call for smithy actions
  ...
  const handleCraft = async (itemToCraft: CraftableItem) => {
    dispatch({ type: 'SET_LOADING', payload: true });

    try {
      // Use real API call for crafting
      const response = await gameApi.performAction('smithy', 'craft', {
        itemId: itemToCraft.id,
        itemName: itemToCraft.name,
        itemType: itemToCraft.type
      });
      
      setDialogMessage(response.message || `The blacksmith has crafted a ${itemToCraft.name} for you!`);
      setIsDialogOpen(true);
      setSelectedService(null);
  ...
  return (
    <div className="p-6 space-y-6 min-h-screen" style={{ fontFamily: "'Cinzel', serif" }}>
      {/* Header */}
      <div className="bg-gradient-to-br from-red-900/90 via-orange-900/80 to-red-900/90 
                      border-2 border-red-700 rounded-lg shadow-[0_0_25px_rgba(239,68,68,0.8)] 
                      backdrop-blur-md p-6 text-center">
        <h2 className="text-4xl font-bold text-red-300 mb-4 drop-shadow-lg tracking-wider">
          The Forge
        </h2>
      ...
      {/* Smithy Scene - Fiery Atmosphere */}
      <div className="bg-gradient-to-br from-red-900/80 via-orange-900/70 to-red-900/80 
                      border-2 border-red-700/80 rounded-lg shadow-[0_0_20px_rgba(234,88,12,0.7)] 
        ...
        <div className="relative text-center mb-6">
          ...
          <h3 className="text-2xl font-bold text-orange-300 tracking-wider drop-shadow">
            Master Blacksmith&apos;s Forge
          </h3>
        </div>
      ...
      {/* Player Gold - Fiery Style */}
      <div className="bg-gradient-to-r from-yellow-900/80 via-amber-900/70 to-yellow-900/80 
                      border-2 border-yellow-600 rounded-lg shadow-[0_0_15px_rgba(234,179,8,0.7)] 
      ...
      {/* Services - Forge-themed buttons */}
      {!selectedService && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {smithyServices.map((service) => {
            const IconComponent = service.icon;
            const canAfford = state.player.gold >= service.baseCost;
            ...
            return (
              <button
                ...
                className={`p-6 rounded-lg border-2 transition-all duration-300 ${
                  canAfford
                    ? 'bg-gradient-to-br from-red-900/80 via-orange-900/70 to-red-900/80 border-red-600 hover:border-orange-500 hover:shadow-[0_0_20px_rgba(249,115,22,0.8)] text-orange-200'
                    : 'bg-gradient-to-br from-gray-800/60 via-gray-700/50 to-gray-800/60 border-gray-600 opacity-50 cursor-not-allowed text-gray-400'
                }`}
              >
      ...
      {/* Equipment Selection for Repair/Upgrade */}
      ...
      {selectedService === 'craft_weapon' && (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-xl font-bold text-red-200">Craft Weapons</h3>
            <button
              onClick={() => setSelectedService(null)}
              className="px-4 py-2 bg-gray-600 hover:bg-gray-500 text-white rounded-lg transition-colors"
            >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {craftableWeapons.map((weapon) => (
              <div
                key={weapon.id}
                className="p-4 rounded-lg border-2 border-red-600/50 bg-gradient-to-b from-red-800/20 to-orange-800/20"
              >
                <div className="text-center space-y-2">
                  <Sword className="w-8 h-8 text-red-400 mx-auto" />
                  <h4 className="font-semibold text-red-200">{weapon.name}</h4>
                  ...
                  <div className="text-xs text-gray-400">
                    Materials: {weapon.materials.join(', ')}
                  </div>
                  <div className="flex items-center justify-center space-x-1">
                    <Coins className="w-4 h-4 text-yellow-400" />
                    <span className="text-yellow-400">{weapon.cost}</span>
                  </div>
                  <button
                    onClick={() => handleCraft(weapon)}
                    disabled={state.player.gold < weapon.cost}
                    className="w-full mt-2 px-4 py-2 bg-red-600 hover:bg-red-500 disabled:bg-gray-600 disabled:cursor-not-allowed text-white rounded-lg transition-colors"
                  >
                    Craft
                  </button>
                </div>
      ...
      {selectedService === 'craft_armor' && (
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="text-xl font-bold text-red-200">Craft Armor</h3>
            <button
              onClick={() => setSelectedService(null)}
              className="px-4 py-2 bg-gray-600 hover:bg-gray-500 text-white rounded-lg transition-colors"
            >
            ...
            {craftableArmor.map((armor) => (
              ...
              >
                <div className="text-center space-y-2">
                  ...
                  <h4 className="font-semibold text-red-200">{armor.name}</h4>
                  <p className="text-sm text-gray-300">{armor.description}</p>
                  <div className="text-xs text-green-400">
                  ...
                  <div className="text-xs text-gray-400">
                    Materials: {armor.materials.join(', ')}
                  </div>
                  <div className="flex items-center justify-center space-x-1">
                    <Coins className="w-4 h-4 text-yellow-400" />
                    <span className="text-yellow-400">{armor.cost}</span>
                  </div>
                  <button
                    onClick={() => handleCraft(armor)}
                    disabled={state.player.gold < armor.cost}
                    className="w-full mt-2 px-4 py-2 bg-red-600 hover:bg-red-500 disabled:bg-gray-600 disabled:cursor-not-allowed text-white rounded-lg transition-colors"
                  >
                    Craft
                  </button>
