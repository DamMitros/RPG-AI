...
import { Building, Users, Sun, Footprints } from 'lucide-react';
...
export default function MainPageLocation() {
  ...
  const handleAction = (action: string) => {
    dispatch({
      ...
      payload: {
        speaker: 'Narrator',
        text: `You perform the action: ${action}`,
      },
    });
  };
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            {[
              ...
              <motion.button key={action} onClick={() => handleAction(action)}
                className="relative bg-gradient-to-br from-yellow-100 to-amber-100 rounded-xl border-3 border-amber-700 shadow-lg p-6 text-left hover:shadow-2xl transition-all duration-300 text-amber-900" style={{
